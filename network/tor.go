...
func SetupTor() (*tor.OnionService, error) {
	...
	// 2. Configure Tor
	// CRITICAL FIX: We removed ExtraArgs entirely.
	// We trust 'bine' to automatically assign random free ports for SOCKS and Control.
	// This prevents the "invalid syntax" error caused by duplicate arguments.
	conf := &tor.StartConf{
		DataDir:     dataDir,
	...
	// Create the onion service listening on port 80
	// RemotePorts: []int{80} -> Maps the Onion Address Port 80 to a random local port
